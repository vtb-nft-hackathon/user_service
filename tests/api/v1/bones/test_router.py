...
async def test_get_bone_by_id(test_client: AsyncClient, bones_repository: WalletRepository) -> None:
    # Arrange
    bone = await bones_repository.add(BoneBase(kind=BoneKind.SKULL, owner_id=1, size=1))

    # Act
    response = await test_client.get(f"/api/v1/wallet/{bone.id}")
...
async def test_create_bone(test_client: AsyncClient) -> None:
    ...
    # Act
    response = await test_client.post("/api/v1/wallet", json=bone.model_dump())
